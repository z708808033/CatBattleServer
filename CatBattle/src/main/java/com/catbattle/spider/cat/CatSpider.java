...
public class CatSpider {
	...
	private static CatInfo getCatData(String catId) {
		...
		try {
			String url = "https://battlecats-db.com/unit/" + catId.substring(0,3) + ".html";		//normal
			int retryTime = 0;
			while(true) {
				boolean flag = true;
				String errMsg = "";
	...
	private static void getCatDataInsertDB(String[] catIds) {
		...
		for (int i = 0; i < catIds.length; i++) {
			try {
				String url = "https://battlecats-db.com/unit/" + catIds[i] + ".html";		//normal
				int retryTime = 0;
...
//		System.out.println(element);
		...
		CatInfo cat = new CatInfo();
		String name = CatPropertyTool.getCatName(element)[index];
